...
console.log(`Sorted employee ID's in Ascending order-------------`);

const sortId=arrayEmployees
  .sort((object1, object2) => {
    return object1.emp_id > object2.emp_id ? 1 : -1;
  })
 sortId.forEach((employee)=>{
...
const sortDept = arrayEmployees.sort((object1, object2)=>{
return object1.emp_dept > object2.emp_dept?1:-1;
})
for (const employee of sortDept) {
    console.log(`ID: ${employee.emp_id}, Department: ${employee.emp_dept}, Company: ${employee.emp_company}`);
}

console.log(`Sorted employee Salary in descending order--------------`);

const sortSalary = arrayEmployees.sort((emp1, emp2)=>{
    return emp1.emp_salary >emp2.emp_salary?-1:1;
})
sortSalary.forEach((employee)=>{
